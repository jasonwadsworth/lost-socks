...
import { signIn, signUp, confirmSignUp, forgotPassword, confirmForgotPassword } from './auth';
import './Login.css';
...
function Login({ onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        if (!confirmCode) { setError('Please enter the confirmation code'); setLoading(false); return; }
        ...
        if (!confirmCode || !newPassword) { setError('Please fill in all fields'); setLoading(false); return; }
  ...
  const getSubtitle = () => {
    ...
    if (mode === 'resetPassword') return 'Enter the code from your email and new password';
        ...
        <form onSubmit={handleSubmit} className="login-form">
          {(mode === 'signin' || mode === 'signup' || mode === 'forgot') && (
            <div className="form-group">
              ...
              <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="you@example.com" disabled={loading} />
            </div>
          )}
          ...
          {(mode === 'signin' || mode === 'signup') && (
            <div className="form-group">
              ...
              <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" disabled={loading} />
            </div>
          )}
          ...
          {(mode === 'confirm' || mode === 'resetPassword') && (
            <div className="form-group">
              ...
              <input type="text" id="confirmCode" value={confirmCode} onChange={(e) => setConfirmCode(e.target.value)} required placeholder="Enter 6-digit code" disabled={loading} />
            </div>
          )}
          ...
          {mode === 'resetPassword' && (
            <div className="form-group">
              ...
              <input type="password" id="newPassword" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required placeholder="••••••••" disabled={loading} />
            </div>
          )}
          ...
          <button type="submit" className="login-button" disabled={loading}>
            ...
            <span className="button-emoji">🧦</span>
              ...
              {(mode === 'forgot' || mode === 'resetPassword') && (
                <div className="signup-link">
                  <a href="#" onClick={(e) => { e.preventDefault(); setMode('signin'); setError(''); }}>Back to Sign In</a>
                </div>
              )}
            </div>
          </div>
